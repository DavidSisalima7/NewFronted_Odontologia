...
const PiezasForm = (props) => {
    //Props
    const { isVisible, setIsVisible, seleccion, toast } = props;
    ...
    const _borrarPieza = () => {
        if (editPieza) {
            ...
            setIsVisible(false);
            setConfirm(false)
            toast.current.show({ severity: 'error', summary: 'Eliminado', detail: 'Datos eliminados' });
        }
    }
    ...
    return (
        <>
            ...
            className="DialogoCentrado"
                ...
                onHide={() => {setIsVisible(false);setEditPieza(false)}}
            >   
            <Divider/>
                <label className="form-label elementosDialog TextoMamalon">Pieza Seleccionada: </label>
                <div className="elementosDialog">
                    <Avatar label={piezaData.numero_pieza} className="mr-2 elementosDialog" size="xlarge" />
                </div>
                <Divider/>
                {/* Tratamiento */}
                <label className="form-label elementosDialog TextoMamalon">Tratamiento:</label>
                <div className="card flex justify-content-center elementosDialog">
                    <Dropdown value={piezaData.tratamiento} onChange={(e) => changeField(e.target.value, "tratamiento")} options={tratamientos} optionLabel=""
                        placeholder={piezaData.tratamiento} className="w-full md:w-20rem " />
                </div>
                <Divider/>
                {/* Caras */}
                <label className="form-label elementosDialog TextoMamalon">Cara:</label>
                <div className="card flex justify-content-center elementosDialog">
                    <Dropdown value={piezaData.cara_pieza} onChange={(e) => changeField(e.target.value, "cara_pieza")} options={caras} optionLabel=""
                        placeholder="Seleccione un tratamiento" className="w-full md:w-14rem" />
                </div>
                <Divider/>
                <div>
                    <Button label="Aceptar" icon="pi pi-check" onClick={guardarPieza} autoFocus />
                    <Button label="Borrar" icon="pi pi-times" onClick={() => setConfirm(true)} className="p-button-text" />
                </div>
            </Dialog>
            ...
            <Dialog header="Desea eliminar este registro?" visible={confirm} style={{ width: '25vw' }} onHide={() => setConfirm(false)}>
                <div>
                    <Button label="Cancelar" icon="pi pi-times" onClick={() =>{setIsVisible(false);setEditPieza(false);setConfirm(false)}} className="p-button-text" />
                    <Button label="Confirmar" icon="pi pi-check" onClick={_borrarPieza} autoFocus />
                </div>
            </Dialog>
        </>
    );
